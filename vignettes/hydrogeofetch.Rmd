...
if(local) {
  cache_path <- file.path(hydrogeofetch_data_dir(), "nhdpt_v_cache")
} else {
  cache_path <- tempdir()
}
...
Or fetch an NWIS site as the starting point and generate a plot. Data is returned and/or stored in a local file for later use.
```{r tldr2}
...
This vignette covers a range of utilities the hydrogeofetch package offers for working with data in a U.S. context.

The first thing you are going to need to do is go get some data to work with. `hydrogeofetch` provides the ability to download small subsets of the NHDPlus directly from web services. For large subsets, greater than a few thousand square kilometers, you can use `download_nhdplusv2()`.

If you are working with the whole National Seamless database, `hydrogeofetch` has some convenience functions you should be aware of. Once you have it downloaded and extracted, you can tell the hydrogeofetch package where it is with the `nhdplus_path()` function.

This vignette stages its data in a working directory. Set `work_dir` to
wherever you want downloaded and sample data to live.

```{r work_dir, echo=TRUE}
work_dir <- file.path(hydrogeofetch_data_dir(), "nhdpt_v_cache")
...
NHDPlus HiRes is an in-development dataset that introduces much more dense flowlines and catchments. In the long run, `hydrogeofetch` will have complete support for NHDPlus HiRes. So far, `hydrogeofetch` will help download and interface NHDPlus HiRes data with existing `hydrogeofetch` functionality. It's important to note that `hydrogeofetch` was primarily implemented using NHDPlusV2 and any use of HiRes should be subject to scrutiny.

For the demo below, a small sample of HiRes data is staged into `work_dir` (the working directory set above) and read from there. The first line shows how you can download additional data (just change `download_files` to `TRUE`).
...
One of the primary workflows `hydrogeofetch` is designed to accomplish can be described in three steps:
...
**If you have the whole National Seamless database and want to work at regional to national scales, skip down to the Local Data Subsetting section.**

### Web Service Data Subsetting

`hydrogeofetch` supports discovery and data subsetting using web services made available through the [Network Linked Data Index](https://waterdata.usgs.gov/blog/nldi-intro/) (NLDI) and the [Water Mission Area pygeoapi](https://api.water.usgs.gov/fabric/pygeoapi). The code below shows how to use the NLDI functions to build a dataset upstream of our `start_comid` that we found above.
...
The NLDI only provides geometry and a comid for each of the flowlines. The `subset_nhdplus()` function has a "download" option that allows us to download four layers and all attributes as shown below.
...
While web service data access is very convenient, some use cases make working with web services impossible or cumbersome such that working with local data is preferable. hydrogeofetch supports such workflows with hybrid, web-service and local, workflows.

With the starting COMID we found with `discover_nhdplus_id()` above, we can use hydroloom's `navigate_hydro_network()` to retrieve a collection of COMIDs along the upstream-with-tributaries network path.
...
Indexing functions have moved to the [hydroloom](https://doi-usgs.github.io/hydroloom/) package. Use hydrogeofetch to fetch flowline data and hydroloom to index points to those flowlines.

```{r indexing}
hydroloom::index_points_to_lines(flowline, start_point)
```
...
hydroloom::index_points_to_lines(flowline, sf::st_geometry(gage), precision = 10)
```

See the [hydroloom indexing documentation](https://doi-usgs.github.io/hydroloom/reference/index.html#indexing-and-linear-referencing) and the [migrating from nhdplusTools article](https://doi-usgs.github.io/hydrogeofetch/articles/migrating_from_nhdplusTools.html) for more.
