...
<https://doi-usgs.github.io/hydrogeofetch/articles/drainage_area_estimation.html>.*
...
7. **Dissolve the NHDPlusV2 basin (optional).** The catchment polygons
   for the upstream network are dissolved into a single boundary,
   `drainage_basin`, with the outlet catchment replaced by the upstream
   side of its split so the portion below the gage is not included. No
   HU boundaries take part. This needs the upstream catchment polygons,
   so it requires `catchments = TRUE` or a `catchment_data` table --
   except for headwater starts, where the gap zone is already the whole
   upstream network and the basin comes back either way.

8. **Assemble scalar estimates.** Six drainage area estimates are
   computed: total and contributing at the HU12, HU10, and HU8
   query levels. Each equals the HU12 polygon area sum plus the gap
   area. Two more scalars serve as comparison baselines.
   `network_da_sqkm` is the NHDPlusV2 cumulative drainage area
   *attribute* at the outlet, which covers the whole outlet catchment.
   `basin_da_sqkm` is the *geometric* area of `drainage_basin`, with the
   below-gage portion removed. The two differ by that portion plus the
   usual drift between a published attribute and a measured geometry.

9. **NHDPlus High Resolution (NHDPlusHR) estimate (optional).** When
...
In a headwater catchment the split carries more weight than elsewhere.
Nothing drains to the gage from further upstream, so the upstream side
of the split is the entire basin by construction, and the drainage area
is taken from that one polygon rather than assembled from HU12 areas
and a gap.
...
- **NHDPlusV2 OGC API** (`get_nhdplus`): fetches flowline attributes,
  ...
  the full upstream catchment polygon set. That full set is also what
  the dissolved `drainage_basin` is built from, so `catchments = TRUE`
  buys both. Skipped for flowline attributes when
...
Each basin result is stored in a separate RDS file in the hydrogeofetch
...
fetch_or_load <- function(name, start) {
  # v2 caches carry drainage_basin / basin_da_sqkm. Bumping the name
  # retires pre-basin caches rather than reading past their missing
  # elements.
  rds_path <- file.path(data_dir,
    paste0("da_est_v2_", name, ".rds"))
...
summary_df <- data.frame(
  ...
  basin_da = vapply(da_results,
    \(x) ifelse(is.na(x$basin_da_sqkm), NA_real_, x$basin_da_sqkm),
    numeric(1)),
...
knitr::kable(summary_df, digits = 1,
  col.names = c("Basin", "Network DA", "Basin DA", "HU12 DA",
...
tile_cache <- file.path(hydrogeofetch_data_dir(), "da_v_tiles")
