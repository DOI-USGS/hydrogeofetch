...
if(local) {
  cache_path <- file.path(hydrogeofetch_data_dir(), "3dhp_v_cache")
} else {
  cache_path <- tempdir()
}
...
This vignette shows how to work with a [new service based on data from the 3D Hydrography Program (3DHP)](https://3dhp.nationalmap.gov/arcgis/rest/services/usgs_3dhp_all/FeatureServer) released in 2024. 3DHP data uses "mainstem" identifiers as the primary river ID. We'll work with one for the Baraboo River in Wisconsin.
...
```{r, message=FALSE}
  ...
  plot_nhdplus(bbox = sf::st_bbox(sf::st_buffer(flowline, 500)), plot_config = list(flowline = list(col = NULL)), zoom = 14)
...
With this functionality, we have what we need to inter operate between older NHD data and newly released 3DHP data. This service is brand new and `hydrogeofetch` is just getting going with functionality to support what's shown here. Please [submit issues](https://github.com/DOI-USGS/hydrogeofetch/issues) if you find problems or have questions!!
